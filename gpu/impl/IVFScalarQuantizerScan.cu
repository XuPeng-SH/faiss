 ...
 */
 ...
 template <bool L2, typename T>
 struct IVFScalarQuantizerScan<0, L2, T> {
   static __device__ void scan(float* query,
                               void* vecData,
                               int numVecs,
                               int dim,
 ...
 void
runIVFScalarQuantizerScanTile(Tensor<float, 2, true>& queries,
                    Tensor<int, 2, true>& listIds,
 ...
 void
 runIVFScalarQuantizerScan(Tensor<float, 2, true>& queries,
                Tensor<int, 2, true>& listIds,
                thrust::device_vector<void*>& listData,
                thrust::device_vector<void*>& listIndices,
                IndicesOptions indicesOptions,
   ...
   for (int query = 0; query < queries.getSize(0); query += queryTileSize) {
     ...
     auto outIndicesView =
       ...
       runIVFScalarQuantizerScanTile(queryView,
                        listIdsView,
                        listData,
                        listIndices,
                        indicesOptions,
