 ...
 */
 ...
 #include "IVFScalarQuantizer.cuh"
 #include "../GpuResources.h"
 #include "FlatIndex.cuh"
 #include "InvertedListAppend.cuh"
 #include "IVFScalarQuantizerScan.cuh"
 ...
 IVFScalarQuantizer::IVFScalarQuantizer(GpuResources* resources,
                  FlatIndex* quantizer,
                  bool l2Distance,
                  bool useFloat16,
 ...
 int
 IVFScalarQuantizer::classifyAndAddVectors(Tensor<float, 2, true>& vecs,
 ...
 void
 IVFScalarQuantizer::query(Tensor<float, 2, true>& queries,
                int nprobe,
                int k,
 ...
 std::vector<float>
 IVFScalarQuantizer::getListVectors(int listId) const {
   FAISS_ASSERT(listId < deviceListData_.size());
